...
World::World(const int &param_file, const bool &display_plot, const bool &score_run, const std::vector<cv::String> &task_selection_method ) {
	...
	if (this->score_run) {
		...
		std::ofstream record_out(result_file_name);
		record_out.close();
	}
...
void World::score_and_record_all() {
	...
	if (record_out.is_open()) {
		...
		record_out << this->get_time_step_collected_reward() << "\n";
	}
	record_out.close();
}
...
double World::get_time_step_open_reward() {
	...
	for (int i = 0; i < this->n_nodes; i++) {
		if (this->nodes[i]->is_active()) {
			double task_reward = this->nodes[i]->get_reward_at_time(this->c_time);
...
double World::get_time_step_collected_reward() {

	double time_step_reward = 0.0;
	for (int i = 0; i < this->n_agents; i++) {
		time_step_reward += this->agents[i]->get_collected_reward();
	}
	return time_step_reward;
}
...
void World::iterate_all() {
	// create new tasks as needed and update task status list
	this->generate_tasks();
...
void World::display_world(const int &ms) {
	...
	if (this->PRM_Mat.empty()) {
		this->PRM_Mat = cv::Mat::zeros(int(this->map_width), int(this->map_height), CV_8UC3);
		...
		// draw nodes
		for (int i = 0; i < this->n_nodes; i++) {
			cv::circle(this->PRM_Mat, this->nodes[i]->get_loc(), 5, cv::Scalar(255, 255, 255), -1);
		}
	...
	char map_name[200];
	sprintf_s(map_name, "Map Name: %i", this->rand_seed);

	cv::putText(map, map_name, cv::Point2d(500.0, this->map_height + 80), CV_FONT_HERSHEY_COMPLEX, 1.0, white, 3);
...
void World::initialize_agents() {
	...
	for (int i = 0; i < this->n_agent_types; i++) {
		...
		bool obs_toll = false;
		if (this->p_pay_obstacle_cost > this->rand_double_in_range(0.0, 1.0)) {
			obs_toll = true;
		}
		agent_obstacle_costs.push_back(obs_toll);
	...
	for (int i = 0; i < this->n_agents; i++) {
		...
		Agent* a = new Agent(loc, i, type, agent_travel_vels[type], agent_colors[type], agent_obstacle_costs[type], this);
...
void World::initialize_PRM() {
	...
	for (int i = 0; i < this->n_nodes; i++) {
		...
		this->k_map_connections = min(this->k_map_connections, this->n_nodes-1);
