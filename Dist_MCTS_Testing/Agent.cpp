...
Agent::Agent(const int &loc, const int &index, const int &type, const double &travel_vel, const cv::Scalar &color, const bool &pay_obstacle_cost, World* world_in){
	this->world = world_in;
...
void Agent::move_along_edge() {
	...
	double dist = 0;
	if (this->world->get_edge_cost(this->edge.x, this->edge.y, this->pay_obstacle_cost, dist) || this->edge.x == this->edge.y) {
		if (dist == 0) {
			this->edge_progress = 1.0;
